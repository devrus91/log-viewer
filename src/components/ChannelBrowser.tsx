...
import { Check, ChevronsUpDown, Eye, EyeOff, FunctionSquare, Pencil, Pin, Search, Settings2 } from "lucide-react";
import { datasetStore } from "@/data/dataset-store";
import { useWorkspaceStore } from "@/state/workspace-store";
import { formatChartValue } from "@/components/charts/chart-preferences";

type SidebarTab = "channels" | "diagnostics" | "presets" | "info";
...
export function ChannelBrowser({ onOpenRules, onEditCalculated }: { onOpenRules?: () => void; onEditCalculated?: (channelId: string) => void }) {
  ...
  const current = (id: string) => formatChartValue(cursorIndex === null ? undefined : datasetStore.getColumn(id)?.[cursorIndex], valuePrecision);
  const sidebarTabs: SidebarTab[] = diagnosticsEnabled ? ["channels", "diagnostics", "presets", "info"] : ["channels", "presets", "info"];
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => { if ((event.metaKey || event.ctrlKey) && event.key.toLowerCase() === "k") { event.preventDefault(); searchRef.current?.focus(); } };
    window.addEventListener("keydown", handleKeyDown); return () => window.removeEventListener("keydown", handleKeyDown);
  }, []);

  return <aside className="sidebar">
    <div className={`sidebar-tabs ${diagnosticsEnabled ? "" : "three-tabs"}`} aria-label="Sidebar sections">{sidebarTabs.map((tab) => <button key={tab} aria-pressed={sidebarTab === tab} className={sidebarTab === tab ? "active" : ""} onClick={() => setSidebarTab(tab)}>{tab === "info" ? "Log info" : tab[0].toUpperCase() + tab.slice(1)}</button>)}</div>
    {sidebarTab === "channels" && <>
      <div className="search-box"><Search size={14} /><input ref={searchRef} aria-label="Search channels" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search 100–500+ channels" /><kbd>Ctrl/⌘+K</kbd></div>
      ...
      <div className="channel-list">{Array.from(groups.entries()).map(([group, items]) => <div className="channel-group" key={group}>
        ...
        {!collapsed.includes(group) && items.map((channel) => { const selected = selectedChannelIds.includes(channel.id); const hidden = hiddenChannelIds.includes(channel.id); const pinned = pinnedTooltipChannelIds.includes(channel.id); return <div className={`channel-row ${selected ? "selected" : ""} ${activeChannelId === channel.id ? "active-channel" : ""}`} key={channel.id} draggable onDragStart={(event) => event.dataTransfer.setData("channel/id", channel.id)}>
          ...
          {selected && <button className={`channel-action tooltip-pin ${pinned ? "active" : ""}`} aria-label={`${pinned ? "Unpin" : "Pin"} ${channel.name} in tooltip`} aria-pressed={pinned} title={`${pinned ? "Unpin from" : "Pin in"} compact tooltip`} onClick={() => toggleTooltipPinnedChannel(channel.id)}><Pin size={12} fill={pinned ? "currentColor" : "none"} /></button>}
