...
export function ImportScreen() {
  ...
  return <main className="import-screen"><nav className="brand"><div className="brand-mark"><Activity size={20} /></div><div><b>AUTOMOTIVE</b><span>LOG VIEWER</span></div></nav><section className="import-card"><div className="import-kicker"><span>LOCAL TELEMETRY WORKSPACE</span><i /></div><h1>Every channel.<br /><em>One clear signal.</em></h1><p>Inspect ECU logs with synchronized plots, calculated channels and high-density heatmaps — without uploading a single byte.</p>
    <div className={`dropzone ${dragging ? "dragging" : ""}`} onDragOver={(event) => { event.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={(event) => { event.preventDefault(); setDragging(false); const file = event.dataTransfer.files[0]; if (file) void load(file); }} onClick={() => inputRef.current?.click()}><div className="drop-icon"><FileUp size={26} /></div><b>{loading ? "PARSING TELEMETRY…" : "DROP CSV LOG HERE"}</b><span>or click to browse · comma, semicolon or tab delimited</span><input ref={inputRef} type="file" accept=".csv,text/csv" onChange={(event) => { const file = event.target.files?.[0]; if (file) void load(file); }} /></div>
    {error && <div className="import-error">{error}</div>}<button className="sample-button" onClick={sample}><Zap size={14} /> Explore with a sample dyno pull</button>
