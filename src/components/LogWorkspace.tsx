...
import { Activity, Axis3D, BookOpen, ChevronDown, ChevronUp, Columns3, Expand, FilePlus2, Flame, FunctionSquare, Grid3X3, Info, Maximize2, Pencil, Plus, RotateCcw, Save, Settings2, Table2, Trash2 } from "lucide-react";
...
export function LogWorkspace() {
  ...
  return <div className="app-shell" ref={workspaceRef}>
    <header className="topbar"><div className="workspace-brand"><WotLabBrand compact /></div><div className="file-crumb"><span>LOGS</span><i>/</i><b>{state.metadata?.filename}</b><small>{state.metadata?.rows.toLocaleString()} samples · {state.metadata?.channels} channels · {state.metadata?.duration.toFixed(1)}s</small></div>
      <div className="top-actions"><button onClick={openAnotherLog} title="Choose another log"><FilePlus2 size={15} /></button><button onClick={() => void saveView()}><Save size={15} /> Save view</button><Link className="top-docs-link" href="/docs" title="Documentation"><BookOpen size={15} /><span>Docs</span></Link><button onClick={() => workspaceRef.current?.requestFullscreen()} title="Fullscreen"><Maximize2 size={15} /></button><button className={settingsOpen ? "active" : ""} aria-label="Viewer settings" title="Settings" onClick={() => setSettingsOpen(true)}><Settings2 size={15} /></button></div></header>
