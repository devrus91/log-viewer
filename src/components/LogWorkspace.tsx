...
import { Activity, Axis3D, ChevronDown, ChevronUp, Columns3, Expand, FilePlus2, Flame, FunctionSquare, Grid3X3, Info, Maximize2, Plus, RotateCcw, Save, Settings2, Table2, Trash2 } from "lucide-react";
...
export function LogWorkspace() {
  ...
  useEffect(() => {
    const handler = (event: KeyboardEvent) => {
      if ((event.metaKey || event.ctrlKey) && event.key.toLowerCase() === "f") { event.preventDefault(); setFormulaOpen(true); }
      if (event.key === "Escape") { setFormulaOpen(false); setSettingsOpen(false); setRulesOpen(false); }
  ...
  return <div className="app-shell" ref={workspaceRef}>
    ...
    <div className="toolbar"><div className="mode-switcher">{MODES.map(({ id, label, icon: Icon }) => <button key={id} className={state.mode === id ? "active" : ""} onClick={() => state.setMode(id)}><Icon size={14} />{label}<kbd>{MODES.findIndex((mode) => mode.id === id) + 1}</kbd></button>)}</div><div className="toolbar-divider" /><label className="compact-select"><span>X AXIS</span><select value={state.xChannelId} onChange={(event) => state.setXChannel(event.target.value)}>{state.channels.map((channel) => <option value={channel.id} key={channel.id}>{channel.name}</option>)}</select></label><div className="toolbar-divider" />
      ...
      <button onClick={state.resetZoom}><RotateCcw size={14} /> Reset zoom</button><button className="formula-button" onClick={() => setFormulaOpen(true)}><FunctionSquare size={14} /> Formula <kbd>⌘F</kbd></button></div>
    <DiagnosticsSummary />
    ...
    </main><ChannelBrowser onOpenRules={() => setRulesOpen(true)} /></div>
    <DiagnosticDetails />
    ...
    <FormulaBuilder open={formulaOpen} onClose={() => setFormulaOpen(false)} />{toast && <div className="toast"><Save size={15} />{toast}</div>}
  </div>;
}
